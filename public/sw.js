...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(self.registration.showNotification(data.title || 'QueueUp', {
    body: data.body,
    data: { url: data.url || (self.location.pathname.startsWith('/queueup') ? '/queueup/' : '/') },
  }));
});

self.addEventListener('notificationclick', (event) => {
  event.notification.close();
  const url = event.notification?.data?.url || (self.location.pathname.startsWith('/queueup') ? '/queueup/' : '/');
